...
def test(G, D, G_ema, z_, y_, state_dict, config, sample, get_inception_metrics,
  ...
  if config['accumulate_stats']:
    utils.accumulate_standing_stats(G_ema if config['ema'] and config['use_ema'] else G,
                           z_, y_, config['n_classes'],
                           config['num_standing_accumulations'])
  IS_mean, IS_std, FID, PRDC = get_inception_metrics(sample, 
  ...
  test_log.log(itr=int(state_dict['itr']), IS_mean=float(IS_mean),
               ...
               Precision=float(PRDC['precision']), Recall=float(PRDC['recall']),
               Density=float(PRDC['density']), Coverage=float(PRDC['coverage']))