...
def compute_prdc(real_features, fake_features, nearest_k):
  ...
  Recommended number of sampels is 10000, with k=5
...
def prepare_inception_metrics(dataset, parallel, no_fid=False):
  ...
  real_features = np.load(dataset+'_inception_activations.npz')['real_features']
  # Load network
  ...
  def get_inception_metrics(sample, num_inception_images, num_splits=10, 
    ...
    prdc = compute_prdc(real_features, pool[:10000].cpu().numpy(), nearest_k=5)
    if no_fid:
      FID = 9999.0
    else:
      if prints:
    ...
    return IS_mean, IS_std, FID, prdc
